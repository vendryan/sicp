  ...
  (define (encode-symbol-1 tree)
    (cond ((leaf? tree) '())
          ((exist-symbol? symbol (symbols (right-branch tree))) ; Check if exist in right branch
            (cons 1 (encode-symbol-1 (right-branch tree)))) ; if yes cons 1 with the rest of th encode of right branch
          (else (cons 0 (encode-symbol-1 (left-branch tree)))))) ; otherwise it must exist in left branch so cons 0 with rest of the encode of left branch
  (if (not (exist-symbol? symbol (symbols tree)))
      (error "bad symbol -- ENCODE-SYMBOL" symbol)
      (encode-symbol-1 tree)))

; searched internet, i'm so stupid
...
(print (generate-huffman-tree '((A ) (B 2) (C 1) (D 1))))
